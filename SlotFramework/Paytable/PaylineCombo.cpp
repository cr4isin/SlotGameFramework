...
Combo PaylineCombo::GetCombo(const vector<string>& combo) const {
	vector<int> comboKey;
	for (const string& symbol : combo)
	{
		comboKey.push_back(symbolSet[symbol]);
	}
	size_t symbolKey = GetSymbolKey(comboKey);
