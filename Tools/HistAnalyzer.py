...
def main():
    ...
    for filename in hist_output_files:

        simname = filename[10:-4]

        # Creating workbook
        workbook = xlsxwriter.Workbook(f'HistResults_{simname}.xlsx', {
        ...
        with open(filename) as file:
            for name, pays, counts in batched(file.read().splitlines(),3):
                pays = list(map(float,pays.strip().split('\t')))
                counts = list(map(int,counts.strip().split('\t')))
                histograms.append([name, pays, counts])
        ...
        for sheetname, pays, counts in histograms:
